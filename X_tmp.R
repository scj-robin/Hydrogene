...
names(Covar)
Covar = Covar[, c(3:13,14:26,27,30)] 
n = nrow(Covar); p = ncol(Covar)
head(Covar)
#summary(Covar)

# 1 = geographie, 2 = physique, 3= chimie,4=Genocenose
...
Group=c(rep(1,19),2,2)                        #
G = max(Group)   
#GroupName = c('geographie','physique', 'chimie','GenoHorsTest','GenoTest')
GroupName = c('Xtmp','GenoTest')
